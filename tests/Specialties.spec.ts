...
test('Validate E2E scenario of Specialties', async ({ request }) => {
  ...
  const createResponse = await specialtiesapi.createPetType(testData.createdSpecialties);
  ...
  const updatedSpecialties = await specialtiesapi.updatePetType(createdSpecialtiesType.id, testData.updatedPetSpecialties);
  expect(updatedSpecialties.ok()).toBeTruthy();

  const deleteSpecialties = await specialtiesapi.deletePetType(createdSpecialtiesType.id);
  expect(deleteSpecialties.ok()).toBeTruthy();

  const remainingPetTypes: Array<{ id: number; name: string }> = await specialtiesapi.getPetTypes();
